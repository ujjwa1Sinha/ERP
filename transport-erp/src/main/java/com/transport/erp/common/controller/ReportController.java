...
public class ReportController {
        ...
        private ReportData getReportData(String entityType) {
                return switch (entityType.toLowerCase()) {
                        ...
                        case "fuel" -> buildFuelReport();
                        case "maintenance" -> buildMaintenanceReport();
        ...
        private ReportData buildFuelReport() {
                List<String> columns = List.of(
                                "date", "vehicle", "driver", "trip_number",
                                "litres", "price_per_litre", "total_amount",
                                "fuel_station", "odometer_reading");

                var fuel = fuelService.getAllFuelTransactions(org.springframework.data.domain.PageRequest.of(0, 10000,
                                org.springframework.data.domain.Sort
                                                .by(org.springframework.data.domain.Sort.Direction.DESC, "date")));
                List<Map<String, Object>> data = fuel.getContent().stream()
                                .map(f -> {
                                        Map<String, Object> row = new LinkedHashMap<>();
                                        row.put("date", f.getDate());
                                        row.put("vehicle", f.getVehicleRegistrationNumber());
                                        row.put("driver", f.getDriverName());
                                        row.put("trip_number", f.getTripRoute());
                                        row.put("litres", f.getLitres());
                                        row.put("price_per_litre", f.getPricePerLitre());
                                        row.put("total_amount", f.getTotalAmount());
                                        row.put("fuel_station", f.getFuelStation());
                                        row.put("odometer_reading", f.getOdometerReading());
                                        return row;
                                }).collect(Collectors.toList());

                return new ReportData("Fuel Report", columns, data);
        }

        private ReportData buildMaintenanceReport() {
                List<String> columns = List.of(
                                "service_date", "vehicle", "maintenance_type",
                                "odometer_reading", "vendor", "cost",
                                "next_service_date", "next_service_odometer");

                var maintenance = maintenanceRecordService.getAllMaintenanceRecords(
                                org.springframework.data.domain.PageRequest.of(0, 10000,
                                                org.springframework.data.domain.Sort
                                                                .by(org.springframework.data.domain.Sort.Direction.DESC,
                                                                                "serviceDate")));
                List<Map<String, Object>> data = maintenance.getContent().stream()
                                .map(m -> {
                                        Map<String, Object> row = new LinkedHashMap<>();
                                        row.put("service_date", m.getServiceDate());
                                        row.put("vehicle", m.getVehicleRegistrationNumber());
                                        row.put("maintenance_type", m.getMaintenanceType());
                                        row.put("odometer_reading", m.getOdometerReading());
                                        row.put("vendor", m.getVendor());
                                        row.put("cost", m.getCost());
                                        row.put("next_service_date", m.getNextServiceDate());
                                        row.put("next_service_odometer", m.getNextServiceOdometer());
                                        return row;
                                }).collect(Collectors.toList());

                return new ReportData("Maintenance Report", columns, data);
        }
